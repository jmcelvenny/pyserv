		...
		if (os.path.isfile(fpath)):
			if (os.access(fpath, os.R_OK)):
				if (filename[-3:] == ".py"):
					...
					sendWriteHeaders(conn, time.strftime("%c"), "text/html", len("python file activated"))
					#conn.send(python_file_output)
					conn.send("python file activated")
				else:
					f = open(fpath, 'r')
					filedata = f.read()
					conn.send("HTTP/1.1 200 OK\n")
			...
			if (os.path.isfile(fpath)):
				...
				sendErrorPageAndClose(conn, "Index Not Found", 404)
		else:
			sendErrorPageAndClose(conn, "Not Found 2", 404)
